...
public class PlayerLocomotion : MonoBehaviour
{
    ...
    {
        Vector3[] trajectoryPoints = CalculateTrajectory(PoopLaunchPoint.position, Vector3.down * PoopLaunchSpeed);

        lineRenderer.positionCount = trajectoryPoints.Length;
        lineRenderer.SetPositions(trajectoryPoints);
    }

    public Vector3[] CalculateTrajectory(Vector3 startPosition, Vector3 initialVelocity)
    {
        ...
        {
            float t = i * timeStep;
            Vector3 gravity = Physics.gravity * t * t / 2.0f;
            Vector3 point = startPosition + t * initialVelocity + gravity;
            points[i] = point;
        }
